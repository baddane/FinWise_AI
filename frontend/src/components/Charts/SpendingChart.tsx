  ...
  return (
    <div className="card p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-sm font-semibold text-surface-900">{t("charts.incomeVsExpenses")}</h3>
          <p className="text-xs text-surface-400 mt-0.5">{t("charts.monthlyComparison")}</p>
        </div>
        <div className="flex items-center gap-4 text-xs">
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-sm bg-brand-500" />
            <span className="text-surface-500 font-medium">{t("charts.income")}</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-sm bg-red-400" />
            <span className="text-surface-500 font-medium">{t("charts.expenses")}</span>
          </div>
        </div>
      </div>
      ...
      {hasData ? (
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={data} margin={{ top: 0, right: 0, left: -10, bottom: 0 }} barGap={4}>
            ...
            <Bar dataKey="income" fill="#10b981" name={t("charts.income")} radius={[6, 6, 0, 0]} maxBarSize={32} />
            <Bar dataKey="expenses" fill="#f87171" name={t("charts.expenses")} radius={[6, 6, 0, 0]} maxBarSize={32} />
          </BarChart>
        </ResponsiveContainer>
      ) : (
        <div className="h-[280px] flex flex-col items-center justify-center text-surface-400">
          <BarChart3Icon />
          <p className="text-sm font-medium mt-3">{t("charts.noData")}</p>
          <p className="text-xs mt-1">{t("charts.noDataDescription")}</p>
        </div>
      )}
    </div>
  );
